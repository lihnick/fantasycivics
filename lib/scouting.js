...
function ScoutingReport(){
	...
	module.getHistoricalScores = (pid, from, to) => {
		...
		var weeks = [];
		var current = incrementMonth(from, 0);
		while(current < to){
			weeks.push({
				from: current,
				to: incrementMonth(current, 1)
			});
			current = incrementMonth(current, 1);
		}
			...
			Promise.all(promises).then(packets => {
				var scoreList = weeks.map(week => {
					...
					packets.forEach((packet, idx) => {
						...
						var score = Database.Scoring.scoreData(packet, meta.dataset, week.from, week.to);
	...
	module.viewReport = (pid, range) => {
		var player = PLAYER_MAP[pid];
		var hp = module.getHistoricalScores(pid, range.from, range.to);
		hp.then(list => {
			// Project Score
			...
			list.push({
				...
				from: incrementMonth(last.from, 1),
				to: incrementMonth(last.to, 1),
				//scores: {val: next},
				scores: futureScores,
				projection: true
			});
			...
			for(var did in last.scores){
				...
				hd.innerText = Database.Scoring.DATASETS[did].name;
			...
			list.forEach(week => {
				...
				var td2 = document.createElement('td');
					td2.innerText = module.sumScores(week.scores);
					tr.appendChild(td1);
					tr.appendChild(td2);
				for(var did in week.scores){
					var sd = document.createElement('td');
					if(did === 'graffiti'){
						console.log(week.scores)
					}
					sd.innerText = week.scores[did];
					tr.appendChild(sd);
				}
	...
	// Note: this function does not replace existing click event handlers
	module.attachReport = (div, pid, range) => {
	...
	var elCallback = (e) => {
		...
		var from = parseInt(e.target.dataset.from);
		var to = parseInt(e.target.dataset.to);
		if(!pid){
			console.error('Could not find {playerid} in [data-pid].');
		}
		else if(!from){
			console.error('Could not find {from} in [data-from].');
		}
		else if(!to){
			console.error('Could not find {to} in [data-to].');
		}
		else{
			module.viewReport(pid, {from: from, to: to});
		}
	}

	module.initializeReport = (queryStr) => {
		var els = document.querySelectorAll(queryStr);
		for(var i = 0; i < els.length; i++){
			var el = els[i];
			el.removeEventListener('click', elCallback);
			el.classList.add('has-scouting-report');
			el.addEventListener('click', elCallback);
		}
	}
